...
def poly_mul(p1, p2):
    ...
    """
    deg = p1.shape[0] - 1
    # would be 2*deg + 1, but next power of 2 handles the +1
    total_num_pts = 2 * deg
    ...
    product = ff_p1 * ff_p2
    return numpy.round(numpy.absolute(ifft(product))).astype(numpy.int32)


if __name__ == "__main__":
    p1 = numpy.array([1, 2, 3, 4], dtype=numpy.int32)
    p2 = numpy.array([2, 3, 4, 5], dtype=numpy.int32)

    output = poly_mul(p1, p2)
    expected = numpy.array([2, 7, 16, 30, 34, 31, 20, 0])
